...
package com.gofiodevelop.jm.votar2015_1.data;


import android.content.Context;
import android.support.v4.util.Pair;

import com.gofiodevelop.jm.votar2015_1.bd.Coneccion;
...
public class ExampleExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    Context con;
    //Bundle datos = DetallePartido.getIntent().getExtras();
    //detalleTitulo.setText(datos.getString("nombrePeli"));
    //tituloSeleccionado ="Has seleccionado "+ datos.getString("nombrePeli");
    //detalleImagen.setImageResource(datos.getInt("imagenPeli"));

    public ExampleExpandableDataProvider() {


        final String [] titulos ={"Candidato", "Programa","Historia","Informacion"};


        //final String groupItems = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        //final String childItems = "123";
        String []childCandidato ={"Nombre","Edad"};
        String [] childPrograma={"Educacion","Economia","Sociedad","Salud"};
       


        mData = new LinkedList<>();
//int i = 0; i < groupItems.length(); i++
        for (int i = 0; i < titulos.length; i++) {
            //noinspection UnnecessaryLocalVariable
            final long groupId = i;
            final String groupText = titulos[i].toString();  //Character.toString(groupItems.charAt(i));
...
final int groupSwipeReaction = RecyclerViewSwipeManager.REACTION_CAN_SWIPE_LEFT | RecyclerViewSwipeManager.REACTION_CAN_SWIPE_RIGHT;
           ...
           switch (i){
               case 0:
               for (int j = 0; j < childCandidato.length; j++) {
                       long childId = group.generateNewChildId();
                       String childText = childCandidato[j];
  ...
  final int childSwipeReaction = RecyclerViewSwipeManager.REACTION_CAN_SWIPE_LEFT | RecyclerViewSwipeManager.REACTION_CAN_SWIPE_RIGHT;
                   ...
                   break;
               case 1:
                   for (int j = 0; j < childPrograma.length; j++) {
                       long childId = group.generateNewChildId();
                       String childText = childPrograma[j];
